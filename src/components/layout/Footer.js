...
export default function Footer() {
  ...
  const linkClass =
    "group relative text-gray-400 hover:text-white transition-colors duration-300 flex items-center gap-2 text-sm py-1 w-fit";
              ...
              {quickLinks.map((link) => (
                <li key={link.name}>
                  <a href={link.href} className={linkClass}>
                    ...
                    {/* Animated underline */}
                    <span className="absolute bottom-0 left-3 w-0 h-px bg-[#012169] group-hover:w-full transition-all duration-300"></span>
                  </a>
                </li>
              ))}
            </ul>
